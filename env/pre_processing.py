...
class PreProcessing:
    ...
    def preprocess_data(self, observation_data, cone_data=None, last_action=None):
        ...
        # 4b. Cross-track error (signed lateral offset from path)
        wp_dir_2d = np.array([wp_dir[0], wp_dir[1]], dtype=np.float64)
        wp_dir_len = np.linalg.norm(wp_dir_2d)
        if wp_dir_len > 1e-6:
            wp_dir_2d /= wp_dir_len
            wp_perp = np.array([-wp_dir_2d[1], wp_dir_2d[0]])
            delta_2d = observation_data['position'][:2] - observation_data['next_waypoint_position'][:2]
            cross_track_error = float(np.dot(delta_2d, wp_perp))
        else:
            cross_track_error = 0.0

        # 4c. Yaw error to second waypoint (lookahead)
        wp2_pos = observation_data.get('second_waypoint_position', observation_data['next_waypoint_position'])
        wp2_dir = wp2_pos - observation_data['position']
        wp2_yaw = math.atan2(wp2_dir[1], wp2_dir[0])
        wp2_yaw_error = wp2_yaw - vehicle_yaw
        while wp2_yaw_error > math.pi: wp2_yaw_error -= 2 * math.pi
        while wp2_yaw_error < -math.pi: wp2_yaw_error += 2 * math.pi
        ...
        norm_cte = np.clip(cross_track_error / 4.0, -1.0, 1.0)
        norm_wp2_yaw_error = wp2_yaw_error / math.pi
        ...
        # Assemble 'rest' vector (Total: 10 features)
        rest_list = [
            norm_forward_v,
            norm_lateral_v,
            norm_ang_v_z,
        ...
        return neo_observation_data, yaw_error, cross_track_error, wp2_yaw_error
