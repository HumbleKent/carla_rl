...
class CarlaServer:
    ...
    def kill_carla_linux():
        if os.name == 'posix':
            ...
            print('This method is only for Unix systems! Use kill_carla_windows for Windows.')

    @staticmethod
    def kill_carla_windows():
        if os.name == 'nt':
            # Kill by image name to ensure all instances (including orphans) are gone
            subprocess.run(['taskkill', '/F', '/IM', 'CarlaUE4-Win64-Shipping.exe', '/T'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            print('All Carla server instances closed on Windows.')
        else:
            print('This method is only for Windows systems!')
