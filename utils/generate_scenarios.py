...
---------------------
Offline utility that auto-generates training scenario definitions from the cone layout
file (`env/cone_spawn.json`) and writes them to `env/vehicle_spawn.json`.

How it works:
  1. Loads all cone (x, y, z) positions from the JSON layout file.
  2. Clusters nearby cones using a distance-based BFS algorithm (similar to DBSCAN).
  3. For each cluster, finds the two endpoints of the cone track using a diameter heuristic
     (equivalent to an approximate PCA max-spread axis).
  4. Generates two scenarios per cluster — one in each direction (A→B and B→A) — with
     vehicle spawn and target positions offset 8 m beyond the cone track ends.
  5. Saves all scenarios to `env/vehicle_spawn.json`.

Usage:
    python utils/generate_scenarios.py

Run from the project root directory. Requires `env/cone_spawn.json` to exist.
Output overwrites `env/vehicle_spawn.json`.
"""
